...
namespace PTCwebApi.DataService {
    public class ConnectSQL {
        ...
        public static bool NotifInsert (string desc, string from, string[] _to, string link, string toolID) {
            try {
                DataTable dt = new DataTable ();
                dt = ConnectSQL.Execute ("SELECT IsNull(MAX(NOTIF_ID),0) FROM NOTIF");
                int newID = Convert.ToInt32 (ConnectSQL.GetString (dt, 0));

                SqlDataAdapter da = ConnectSQL.GatDataAdapter ("SELECT TOP 1 * FROM NOTIF");
                da.Fill (dt);
