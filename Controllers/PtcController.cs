...
namespace PTCwebApi.Controllers {
    ...
    public class PtcController : ControllerBase {
        ...
        public async Task<ActionResult<Object>> ApiAllPTC (RequestAllModelsPTC model) {
            switch (model.fn) {
                case "FIND TOOL":
                    var _resultFindTool = await new PTCMethods (_mapper, _jwtGenerator).FindTooling (model);
                    return Ok (_resultFindTool);
                case "SCAN LOCATION TO MOVE":
                    var _resultMoveTool = await new PTCMethods (_mapper, _jwtGenerator).MoveTooling (model);
                    return Ok (_resultMoveTool);
                default:
                    return null;
            }
        }
